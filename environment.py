...
class WumpusWorld:
    ...
    def print_position(self):
        """Print the agent's current position and orientation."""
        print(f"Agent is at {self.agent_pos} facing {self.agent_orientation}")
    def execute_action(self, action):
        ...
        if self.agent_pos in self.pit_pos:
            self.score -= 1000
            self.game_outcome = "Fell"
            self.game_over = True
            return
        elif self.agent_pos in self.wumpus_positions:
            self.score -= 1000
            self.game_outcome = "Eaten"
            self.game_over = True
            return
    ...
    def _shoot_arrow(self):
        ...
        print(f"Shooting arrow from {self.agent_pos} towards {self.agent_orientation}")
